...
public enum PassPortErrorCode {
	...
	/** 收货地址对多可以添加5个*/
	CONSIGNEE_ADDRESS_MAX_FIVE(4001, "收货地址最多可添加" + MemberInfoConstant.ConsigneeAddress.MaxCount + "个"),
	...
	/** 邮箱地址为空*/
	EMAIL_IS_NULL(4064, "邮箱地址为空"),
	
	/** 操作正在进行*/
	OPERATOR_IS_DO(4073, "操作正在进行"),
	
	/**初始化用户失败*/
	INIT_USER_FAIL(4074, "初始化用户失败"),
	
	/**您已经绑定过手机*/
	USER_HASH_BIND_MOBILE(4076, "您已经绑定过手机"),
	
	/**您已经绑定过邮箱*/
	USER_HASH_BIND_EMAIL(4077, "您已经绑定过邮箱"),
	
	/**token错误*/
	TOKEN_ERROR(4078, "链接已失效"),
	/** 联合登录值未绑定 */
	UNION_VAL_NOT_BIND(4080, "联合登录尚未绑定"),
	UNION_NOT_NULL(4081, "联合登录值和类型不能为空"),
	UNION_NOT_EXIST(4082, "联合登录信息不存在"),
	
	/** 更换手机号*/
	MODIFY_MOBILE_FAIL(4019, "更换手机号失败 ");
	...
	private PassPortErrorCode(Integer code, String value) {
		this.code = code;
		this.value = value;
	}
	...
	public static String getValue(Integer code){
		...
		for (PassPortErrorCode c : PassPortErrorCode.values()) {
