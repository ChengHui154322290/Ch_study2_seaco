...
public class CouponIssueController extends AbstractBaseController {

    Logger log = LoggerFactory.getLogger(this.getClass());

    @Autowired
    private CouponUserProxy userServiceAO;
    ...
    public String sendList(Model model, CouponSend cdo,
        ...
        try {
            ...
            cdo.setType(CouponSendType.NORMAL.ordinal());
        ...
        return "coupon/coupon_send_list";
    }
    ...
    public String searchCoupon(@RequestParam(value = "selectRow") Integer selectRow,
                               @RequestParam(value = "size", defaultValue = "10") Integer size,
        ...
        if (NumberUtils.isNumber(idStr)) {
            id = NumberUtils.toLong(idStr);
        }
        couponName = StringUtils.isBlank(couponName) ? null : couponName.trim();

        PageInfo<Coupon> pageDO = new PageInfo<>();
        if (id != null || couponName != null) {
            Coupon coupon = new Coupon();
            coupon.setId(id);
            coupon.setCouponName(couponName);
            pageDO = couponProxy.queryValidCoupon(coupon, page, size);
    ...
    @ResponseBody
    public ResultInfo<String> searchCouponName(@RequestParam(value = "couponId") Long couponId) {
        if (null != couponId) {
    ...
    @ResponseBody
    public ResultInfo<CouponDto> searchCouponById(@RequestParam(value = "couponId") Long couponId) {
        if (null != couponId) {
    ...
    public ResultInfo issueCoupon(@RequestParam("batches") String batches,
                                  @RequestParam("users") String userCodes,
                                  @RequestParam("isAll") Integer all,
        ...
        if (all == null || (0 == all && StringUtils.isBlank(userCodes))) {
            return new ResultInfo(new FailInfo("用户编号不能为空"));
        }

        return couponProxy.saveCouponSend(batches, userCodes, all, user.getId(), user.getLoginName(), msgSend, msgContent, name, status, couponsendType, startTime, endTime);
        //return userServiceAO.issueCoupon(batches, userCodes, all, user.getId(), user.getLoginName(), msgSend, msgContent);
    }
    ...
    public ResultInfo approveCoupon(Long couponSendId, HttpServletRequest request, HttpServletResponse response) {
        UserInfo user = getUserInfo();
        return couponProxy.approveCouponSend(couponSendId, user);
    }
    ...
    public ModelAndView toEditCouponSend(@RequestParam(value = "id") Long id) throws Exception {
        ...
        mv.setViewName("/coupon/couponIssueEdit");
    ...
    public ResultInfo issueCouponEdit(@RequestParam("batches") String batches,
        ...
        return couponProxy.updateCouponSend(id, batches, userCodes, all, user.getId(), user.getLoginName(), msgSend, msgContent, name, status, couponsendType, startTime, endTime);
        //return userServiceAO.issueCoupon(batches, userCodes, all, user.getId(), user.getLoginName(), msgSend, msgContent);
    }
