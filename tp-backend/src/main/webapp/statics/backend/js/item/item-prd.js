...
$(function(){
	...
	$('.deletePrdBtn').live('click',function(){
		var $this = $(this);
		$this.closest("tr").detach();
	});
	
	//新建prdid
	$('#newPrdBtn').live('click',function(){
		 var specGroupIds = $(this).attr('params');
		  pageii=$.layer({
            ...
            offset: ['0px', 'auto'],
            iframe: {
                src : domain+'/item/getSpecByItemId.htm?specGroupIds='+specGroupIds
            } 
        });
	});
	...
	function handSave(){
		$("#saveItemPrdsBtn").click(function(){
			...
			$.post("saveItemPrds.htm",{info:infoJson,details:detailJson},showResponse,"text");
		});
	}
	...
	function showResponse(data){
		...
		if(obj.success){//成功
			layer.alert('操作成功', 4,function(){
				location.href='/item/queryPrd.htm?itemId='+obj.data;
			});
	    }else{
	...
	//prd新增
	$("#chooseSpecBtn").live('click',function(){
