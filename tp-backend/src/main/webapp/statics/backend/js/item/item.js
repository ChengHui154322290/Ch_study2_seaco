...
var pageii;
$(function(){
	$('.itemSkubtn').live('click',function(){
		  pageii=$.layer({
            type : 2,
            title: '商品对应的Sku信息->编辑',
            shadeClose: true,
            ...
            area: ['400px', 400],                     
            iframe: {
                src : domain+'/item/skuAdd.htm'
            } 
        });
	}); 
	
	//新建prdid
	$('#newPrdidBtn').live('click',function(){
		  ...
		  pageii=$.layer({
            ...
            area: ['600px', 400],                     
            iframe: {
                src : domain+'/item/addSpec.htm?smallId='+smallId
            } 
        });
	});
	//清空prdid列表
	$('#clearPrdidListBtn').live('click',function(){
		$('#skuListTable').empty();
	});
	...
	$(".newStorageBtn").live("click", function() {
		var $this = $(this);
	...
	// 删除商品对应的供应商 
	$(".deleteSuppListItem").live("click", function() {
		var $this = $(this);
	...
	function handSave(){
		$("#dataFormSave").click(function(){
			...
			$.post("save.htm",{info:infoJson,details:detailJson},showResponse,"text");
		});
	}
	...
	function showResponse(data){
		...
		if(obj.success){//成功
			layer.alert('操作成功', 4,function(){
				location.href='queryPrd.htm?itemId='+obj.data;
				//location.href='add.htm?itemId='+obj.data;
			});
	    }else{
