...
var EDIT_TOPICITEM_AMOUNT = domain + "/topicItemChange/editItem"
$(document).ready(function() {

	$("#save").on("click", function() {
		var topicId = $("#topicId").val();
		var topicItemId = $("#topicItemId").val();
		var topicItemChangeId = $("#topicItemChangeId").val();
		...
		var changeOrderLimitTotal = $("#changeOrderLimitTotal").val();
		...
		if((parseInt(addLimitAmount)+parseInt(changeOrderLimitTotal)) < 1){
		...
		var data = syncPost(EDIT_TOPICITEM_AMOUNT,{
			topicItemChangeId : topicItemChangeId,
		...
		if (data != null) {
			if(data.success){
				...
				parent.$("#topicItemsList .topicItemData").each(function(){
					var id = $(this).find("input[name='changeTopicItemId']").val()
					if(topicItemId == id){
						...
						if(null == currentAmount){
							$(this).find("input[name='limitTotal']").val(parseInt(data.data));
						}else{
							$(this).find("input[name='limitTotal']").val(parseInt(data.data) + parseInt(currentAmount));
						}
						hasChange = true;
