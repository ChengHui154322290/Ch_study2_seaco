var UPLOAD_ITEM_IMAGE = domain + "/topic/uploadCouponImage";
var swfuItem;
...
$(document).ready(function() {
	$("#close").on("click",function(){
		swfuItem = null;
		$("#spanButtonCouponPic").remove();
		parent.layer.close(index);
	});
	createCouponPicUploadControl();
})

function createCouponPicUploadControl() {
	var layerShow;
	...
	var itemSettings = {
		...
		file_queue_limit : 0,
		custom_settings : {
			progressTarget : "couponPicUploadProgress"
		},
		debug : false,
		...
		button_width : "100",
		button_height : "30",
		button_placeholder_id : "spanButtonCouponPic",
			...
			setTimeout(function(){
				try {
					if (!serverData || serverData.length == 0) {
						...
						var itemIndex = $("#selectCouponIndex").val();
						var data = JSON.parse(serverData);
						if (null != data) {
							if (null != itemIndex) {
								var row = parent.$("#topicCouponsList tr")[itemIndex];
								if (null != row) {
									if (null == data.file) {
										var result = eval("(" + data + ")");
										if (null != result) {
											$(row).children().find("input[name='couponImage']").val(result.file);
											$(row).children().find("img[name='selectCouponImage']").attr("src",result.fullPath);
										}
									} else {
										$(row).children().find("input[name='couponImage']").val(data.file);
										$(row).children().find("img[name='selectCouponImage']").attr("src",data.fullPath);
									}
								}
							}
						}
