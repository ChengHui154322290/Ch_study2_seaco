var ISSUE_COUPON = domain + "/coupon/issueCouponEdit";
var SEARCH_COUPON = domain + "/coupon/showCouponSearch";
...
$(document).ready(function() {
	$(".issue").bind("click", function() {
		...
		var id = $("#id").val();
		var all = $("#allUser")[0].checked ? 1 : 0;
		var msgSend = $("#msgSend")[0].checked ? 1 : 0;
		var userCodes = $("#userList").val();
		var msgContent = $("#msgContent").val();
		var name = $("#name").val();
		var couponsendStatus = $(this).attr("status");
		var couponsendType = 0;
		if (!all && (null == userCodes || "" == $.trim(userCodes))) {
		...
		$.post(ISSUE_COUPON, {
			...
			if (data.success) {
				if(couponsendStatus =='0')
					...
					layer.alert("提交成功");
				window.location.href = domain+'/coupon/sendList.htm';
			} else {
...
$(function(){
	...
	if(couponInfos != null){
		if(couponInfos.toAll == 1){
			$("#allUser").attr("checked","checked");
		}
		if(couponInfos.sendMsg == 1){
			$("#msgSend").attr("checked","checked");
		}
