...
$(function(){
	...
	$('.speceditbtn').live('click',function(){	
		var id= $(this).attr('param');						
		  pageii=$.layer({
            ...
            area: ['900px', 600],                     
            iframe: {
                src : domain+'/basedata/specGroup/spec/edit.htm?id='+id
            } 
        });
	}); 
//取消按钮
...
$('.specaddbtn').live('click',function(){				
		  pageii=$.layer({
            type : 2,
            title: '规格->增加',
            shadeClose: true,
            ...
            area: ['900px', 600],                     
            iframe: {
                src : domain+'/basedata/specGroup/spec/add.htm'
            } 
        });
	}); 
...
$("#searchSpecGroup").on('input',function(e){
	searchSpecGroup();
});

if($("#datasubmit").is("input")){
	var butt=$("#datasubmit");
	$.formValidator.initConfig({submitButtonID:"datasubmit",debug:false,onSuccess:function(){specSubmit(butt);},onError:function(){alert("具体错误，请看网页上的提示")}});
	$("#spec").formValidator({onShow:"请输入规格名称",onFocus:"至少1个字符,最多20个字符",onCorrect:"正确"}).inputValidator({min:1,max:20,onError:"你输入的规格名称不合法,请确认"});
...
function specSubmit(button){
	var address= $(button).attr('param');
	$.ajax({
        cache: true,
        type: "POST",
        url:domain+address,
        data:$('#specupdate').serialize(),
        async: false,
...
function deleteSpecGroup(attra){
	var address= $(attra).attr('param');
...
function searchSpecGroup() {
	var txt = $("#searchSpecGroup").val().trim();
