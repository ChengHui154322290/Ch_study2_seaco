...
$(function(){
	...
	PFUNC.initAdvertiseRegion =function(){
		...
		//创建
		$('#templet_add').unbind().bind('click',function(){
			goPage.call(this,"/cms/addTemplet.htm");   
		});
		
		//修改
		$('#templet_upd').unbind().bind('click',function(){
			var params={}; 
			var subject = [];
			params.announceinfo ={};
			var announceflag = 0;
			...
			var announceInfo = {};
			announceInfo.pageNameBak = $('input.pageName').val();
			
			goPage.call(this,"/cms/addTemplet.htm",{params:$.toJSON(params),announceInfo:$.toJSON(announceInfo)});   
		});
		
		//删除
		$('#templet_del').unbind().bind('click',function(){
				var params={}; 
				var subject = [];
				...
				var index = layer.confirm(
		                 "您确定需要删除该条反馈信息吗?",
		                  function(index){
		                	 $.post(domain +"/cms/delTemplet.htm", {params:$.toJSON(params)}, function(redata){
		     					if(redata.isSuccess){
		     			            alert("删除成功",1);
