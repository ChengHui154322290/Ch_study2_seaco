 ...
 window.onload = function() {
 	...
 	fileCountlimit = fileCountlimit-hasUploadCount;
 	var sid = $('#sessionId').val();
 	var settings = {
 		flash_url : domain+"/statics/backend/js/swfupload/swfupload.swf",
 		upload_url: domain+"/uploadFile.htm?bucketname=cmsimg",/*上传到本地文件的url*/
 		...
 		upload_success_handler : function(file, serverData){
 			try {
 				...
 				if (!serverData || serverData.length == 0) {
 					...
 					result = result.data;
 					var path = result.fileUrl;	//路径
 					var allItems = $('#item-pictures');
 					var img = $("<img />");
 					img.attr('src',result.absUrl);
					$("#picSrc").val(path);
