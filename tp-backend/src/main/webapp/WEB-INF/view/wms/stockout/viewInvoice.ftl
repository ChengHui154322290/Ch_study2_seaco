...
'/statics/backend/js/jqueryui/css/cupertino/jquery-ui-1.9.2.custom.min.css'
    ...
    <div id="search_bar" class="mt10">
       <div class="box">
          <div class="box_border">
            <div class="box_top"><b class="pl15">出库单发票</b></div>
          </div>
        </div>
    </div>
    <!-- 查询条件结束 -->
    ...
    <div id="table" class="mt10">
        <div class="box span10 oh">
            ...
            <#if outInvoiceList?default([])?size !=0>
            <#list outInvoiceList as invoice>
	             <tr>
	               <td width="10%" align="center">${invoice_index+1}</td>
	               <td width="15%" align="center">${invoice.orderCode}</td>
	               <td width="10%" align="center">${invoice.invoiceCode}</td>
	               <td width="15%" align="center">${invoice.invoiceNo}</td>
	               <td width="10%" align="center">
	               <!-- ${invoice.invoiceType} -->
	               		<#if '${invoice.invoiceType?if_exists}'=='1'>
							增值税发票
						</#if>
						<#if '${invoice.invoiceType?if_exists}'=='2'>
							普通发票
						</#if>
	               </td>
	               <td width="15%" align="center">${invoice.invoiceTitle}</td>
	               <td width="10%" align="center">${invoice.invoiceAmount}</td>
	               <td width="15%" align="center">${invoice.invoiceContent}</td>
	            </tr>
            </#list>
            </#if>
              </table>
