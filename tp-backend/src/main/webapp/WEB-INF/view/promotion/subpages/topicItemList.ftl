		...
		<#if ("view" != "${mode}")>
			...
			<td colspan="5">
				<input type="button" class="ext_btn ext_btn_submit" value="新增商品" width="180" id="addTopicItem" /> 
					...
					<!--
					<input type="button" class="ext_btn ext_btn_submit" value="批量新增" width="180" id="batchAddTopicItem" /> 
					-->
			</td>
		<#else>
				...
				<#if (topicDetail.topicItemDtoList??)>
					<#list topicDetail.topicItemDtoList as topicItem>
						<tr align="center" style="background-color: rgb(255, 255, 255);" class="topicItemData">
							...
							<td>
								<#if ("view" != "${mode}")>
									<#if ("1" != "${topicDetail.topic.reserveInventoryFlag}")> <!-- 1：预留 0：不预留 -->
										<input type="text" class="input-text lh30" name="limitTotal" value="0" readonly/>
									<#else>
										<input type="text" class="input-text lh30" name="limitTotal" value="${topicItem.limitTotal!}" />
									</#if>
								<#else>
							...
							<td><span><#if topicItem.remainStock??>${topicItem.remainStock}</#if></span></td>
							-->
							...
							<#if ("view" != "${mode}")>
								<td>
								<!--
									<a href="javascript:void(0);" name="editItem">[详情]</a> |
									-->
...
<script>
	$(function(){
		...
		$("#batchAddTopicItem").bind("click",function(){
			// 获取已有sku
			var oldSkus = "";
			$("#topicItemsList input[name=topicItemSku]").each(function(i,ele){
				if(i==0){
					oldSkus += "'"+$(ele).val()+"'";
				}else{
					oldSkus += ",'"+$(ele).val()+"'";
			...
			sortIndex = sortIntexMaxRow.val()||"0";
			
			var url = domain+ "/topic/batchAddItemList.htm?topicId="+topicId+"&sortIndex="+sortIndex+"&brandId="+bid;
			if(1 == parseInt(type)){
				var spus = $(".topicItemData input[name='topicItemSpu']:first").val();
				var supplierId = $(".topicItemData input[name='topicItemSupplierId']:first").val();
