					...
					<div class="tc">
						...
						<table class="tab_temple_info">
							<tr>
								...
								<td>
									<input type="text" name="templeName" maxlength="30" value="${detail.templeName}" rows="3" 
										size="20" class="input-text lh30 templeName">
									<input type="hidden" name="idd" value="${detail.id}" class="tempId">
								</td>
								<td class="td_right">模板路径：</td>
								<td><input type="text" name="templePath" value="${detail.templePath}"
									class="input-text lh30 templePath" size="20" disabled="true"></td>
								...
								<td class=""><span class="fl"> <select
										name="status" class="select status">
											...
											<option value="1" <#if "${detail.status}"=="1"> selected</#if>>未启用</option>
									</select>
								</span></td>
							...
							<tr>
								...
								<td class=""><span class="fl"> <select
										name="type" class="select type">
											<option value="10" <#if "${detail.type}"=="10"> selected</#if>>单品团(pc)</option>
											<option value="15" <#if "${detail.type}"=="15"> selected</#if>>海淘首页(pc)</option>
											<option value="16" <#if "${detail.type}"=="16"> selected</#if>>西客商城首页-热销榜单(pc)</option>
											<option value="17" <#if "${detail.type}"=="17"> selected</#if>>西客商城首页-品牌精选(pc)</option>
											<option value="102" <#if "${detail.type}"=="102"> selected</#if>>单品团(APP)</option>
											<option value="206" <#if "${detail.type}"=="206"> selected</#if>>秒杀-单品团(APP)</option>
											<option value="207" <#if "${detail.type}"=="207"> selected</#if>>WAP-今日精选-单品(APP)</option>
											<option value="208" <#if "${detail.type}"=="208"> selected</#if>>WAP今日精选-专场(APP)</option>
									</select>
								</span></td>
