...
@Service(value="haitaoAppService")
public class HaitaoAppService<T> implements IHaitaoAppService{
	...
	private final static Logger logger = LoggerFactory.getLogger(HaitaoAppService.class);
	...
	public AppIndexAdvertReturnData carouseAdvert() {
		...
		try {
			lst = (List<PictureElement>) queryPageTempletElementInfo(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_CAROUSE_ADVERT);
		} catch (Exception e) {
		...
		processData(lst, appAdvertiseInfoDTO);
	...
	public AppIndexAdvertReturnData carouseDssAdvert() {
		...
		List<PictureElement> lst = new ArrayList<PictureElement>();
		try {
			lst = (List<PictureElement>) queryPageTempletElementInfo(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_DSS_CAROUSE_ADVERT);
		} catch (Exception e) {
			logger.error("APP海淘首页-轮播图查询报错", e);
			e.printStackTrace();
		}
	...
	private void processData(List<PictureElement> lst, List<AppAdvertiseInfoDTO<Object>> appAdvertiseInfoDTO) {
		for(int i=0;i<lst.size();i++){
            ...
            if(AdvertTypeAPPEnum.ACT_ITEMSKU.getValue().equals(ssmode.getActtype())
                ...
                csmode.setInfo(appSingleInfoDTO);
    				...
    				||AdvertTypeAPPEnum.SIGN.getValue().equals(ssmode.getActtype())
    				||AdvertTypeAPPEnum.OFFLINE_GROUP.getValue().equals(ssmode.getActtype())
    				){
    		...
    		}else if(AdvertTypeAPPEnum.SEARCH.getValue().equals(ssmode.getActtype())){
    			AppSingleInfoDTO appSingleInfoDTO = new AppSingleInfoDTO();
    			appSingleInfoDTO.setText(ssmode.getSku());
    			csmode.setInfo(appSingleInfoDTO);
    		}else if(AdvertTypeAPPEnum.BRANDID.getValue().equals(ssmode.getActtype())
    				||AdvertTypeAPPEnum.CATEGORYID.getValue().equals(ssmode.getActtype())){
    			AppSingleInfoDTO appSingleInfoDTO = new AppSingleInfoDTO();
    			appSingleInfoDTO.setSpecialid(ssmode.getActivityid());
    			appSingleInfoDTO.setText(ssmode.getSku());
    			csmode.setInfo(appSingleInfoDTO);
	...
	public AppIndexAdvertReturnData dssFunLab(String channelCode) {
		...
		List<PictureElement> lst = new ArrayList<PictureElement>();
		try {
			lst = (List<PictureElement>) queryPageTempletElementInfo(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_DSS_FUNLAB);
		} catch (Exception e) {
    	...
    	for(int i=0;i<lst.size();i++){
    		PictureElement ssmode = lst.get(i);
    		//店铺分销替换
    		ssmode.setLink(ssmode.getLink().replaceFirst("SHOP", channelCode));
	...
	public AppIndexAdvertReturnData funLab() {
		...
		List<PictureElement> lst = new ArrayList<PictureElement>();
		try {
			lst = (List<PictureElement>) queryPageTempletElementInfo(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_FUNLAB);
		} catch (Exception e) {
			logger.error("APP海淘首页-功能标签查询报错", e);
			e.printStackTrace();
		}
	...
	public AppIndexAdvertReturnData funLabForWorld() {
		...
		List<PictureElement> lst = new ArrayList<PictureElement>();
		try {
			lst = (List<PictureElement>) queryPageTempletElementInfo(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_FUNLAB_FOR_WORLD);
		} catch (Exception e) {
			logger.error("APP海淘首页-功能标签查询报错", e);
			e.printStackTrace();
		}
	...
	public PageInfo<AppSingleCotAllInfoDTO> singleTemplet(int pagestart,int pagesize) {
		...
		try {
			lst = (List<ActivityElement>) queryPageTempletElementPage(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_SINGLETEMP, pagestart,pagesize);
		} catch (Exception e) {
	...
	public PageInfo<AppSingleAllInfoDTO> htTopicAllList(Long promoterid, int pagestart,int pagesize,String ...templates) {
		...
		try {
			...
			if(templates != null && templates.length>0){
				templeCodes.addAll(Arrays.asList(templates));
				lst = (List<ActivityElement>) queryActivityElementsByTempletCodes(null, AppTaiHaoTempletConstant.HAITAO_APP,templeCodes, pagestart,pagesize);
			} else if(promoterid == null) {
				templeCodes.add(AppTaiHaoTempletConstant.HAITAO_APP_SINGLETEMP);
				templeCodes.add(AppTaiHaoTempletConstant.HAITAO_APP_TOPICLIST);
				lst = (List<ActivityElement>) queryActivityElementsByTempletCodes(null, AppTaiHaoTempletConstant.HAITAO_APP,templeCodes, pagestart,pagesize);
			}
			else {
				templeCodes.add(AppTaiHaoTempletConstant.HAITAO_APP_DSS_TOPICLIST);
				lst = (List<ActivityElement>) queryActivityElementsByTempletCodes(promoterid, AppTaiHaoTempletConstant.HAITAO_APP,templeCodes, pagestart,pagesize);
			}
		} catch (Exception e) {
		...
		List<AppSingleAllInfoDTO> retLst = getAppSingleAllInfoDTOs(promoterid, lst,templates);
	...
	public List<AppSingleAllInfoDTO> getAppSingleAllInfoDTOs(Long promoterid, List<ActivityElement> lst,String ...templates) {
		...
		filterSpTopics(topicDetailDTO, templates);
				...
				for(TopicDetailDTO dtomd:topicDetailDTO){
					...
					if(dtomd.getTopic().getType().intValue() == TopicConstant.TOPIC_TYPE_SINGLE){
						...
						if(cmsActivityElementDO.getActivityId().longValue() == topic.getId().longValue()
			    			...
			    			//专场名称
			        		appSingleCotAllInfoDTO.setSpecialName(topic.getName());
							appSingleCotAllInfoDTO.setTopicPoint(topic.getTopicPoint() ==null ? "": topic.getTopicPoint());
							appSingleCotAllInfoDTO.setText(topic.getRemark() == null ? "" :topic.getRemark());
					...
					if(dtomd.getTopic().getType().intValue() == TopicConstant.TOPIC_TYPE_TOPIC
							|| dtomd.getTopic().getType().intValue() == TopicConstant.TOPIC_TYPE_BRAND || dtomd.getTopic().getType().intValue()== TopicType.SUPPLIER_SHOP.ordinal()){
		    			...
		    			if(cmsActivityElementDO.getActivityId().longValue() == topic.getId().longValue()){//找到值，即两个活动id一致情况下
							...
							appSingleInfoDTO.setTopicPoint(topic.getTopicPoint() ==null ? "": topic.getTopicPoint());
							appSingleInfoDTO.setText(topic.getRemark() == null ? "" :topic.getRemark());
...
//			    			List<TopicItem> topicItemList = topicItemService.queryByParam(params);
	...
	/**
	 * 获取全部分销专场时屏蔽为第三方店铺提供的特殊专场
	 *
	 * @param topicDetailDTO
	 * @param templates
	 */
	private void filterSpTopics(List<TopicDetailDTO> topicDetailDTO, String[] templates) {
		if (templates == null || templates.length == 0 || topicDetailDTO == null) {
			return;
		}
		boolean hasDSSTemplate = false;
		for (String s : templates) {
			if (StringUtils.equals(s, AppTaiHaoTempletConstant.HAITAO_APP_DSS_TOPICLIST)) {
				hasDSSTemplate = true;
				break;
			}
		}
		...
		List<TopicPromoter> topicPromoters = topicPromoterService.queryByObject(new TopicPromoter());
		if (org.springframework.util.CollectionUtils.isEmpty(topicPromoters)) {
			return;
		}
		Set<Long> topicIds = new HashSet<>();
		topicPromoters.forEach(new Consumer<TopicPromoter>() {
			@Override
			public void accept(TopicPromoter topicPromoter) {
				topicIds.add(topicPromoter.getTopicId());
			}
		});

		Iterator<TopicDetailDTO> iterator = topicDetailDTO.iterator();
		while (iterator.hasNext()) {
			Topic topic = iterator.next().getTopic();
			if (topic == null) {
				continue;
			}
...
//			}
		...
		try {
			List<String> templeCodes = new ArrayList<>();
			templeCodes.add(AppTaiHaoTempletConstant.HAITAO_APP_DSS_TOPICLIST);
			lst = (List<ActivityElement>) queryActivityElementsByTempletCodesForDss(promoterid, AppTaiHaoTempletConstant.HAITAO_APP,templeCodes, noList, 1,pagesize);
		} catch (Exception e) {
			logger.error("APP海淘首页-专场列表查询报错", e);
			e.printStackTrace();
		}
...
//	            	}	            
	...
	public PageInfo<AppSingleAllInfoDTO> htTopicAllListForDss2(Long promoterid, String channelCode,int pagestart,int pagesize) {
		...
		List<Topic> topicList = promoterTopicService.geBrandTopics(promoterid,channelCode);		
		xToBrands(retLst, topicList);
		
		List<TopicItemDss> topicitemList = promoterTopicService.getBrandTopicItems(promoterid,channelCode);	
		...
		List<TopicItemDss> otheritemList = promoterTopicService.getOtherTopicItems(promoterid,channelCode);
...
//				List<TopicItem> itemList = topicItemService.getTopicItemByTopicId_Top10(brandids);
	...
	public PageInfo<AppSingleAllInfoDTO> htTopicAllListForDssWithMS(Long promoterid,String channelCode, int pagestart,int pagesize) {
		...
		List<TopicItemDss> single = promoterTopicService.getSingleTopicItem(promoterid,channelCode);
		single.removeIf(new Predicate<TopicItemDss>(){
			public boolean test(TopicItemDss topicItemDss) {
				return null!=topicItemDss.getItemStatus() && !ItemStatusEnum.ONLINE.getValue().equals(topicItemDss.getItemStatus()) && -1!=topicItemDss.getItemStatus();
			}
		});
		xtoTopicItems( promoterid,  retLst,  single);
...
//				appSingleCotAllInfoDTO.setCommission( commision );								
	...
	public PageInfo<AppSingleInfoDTO> htTopicList(int pagestart,int pagesize) {
		...
		List<ActivityElement> lst = new ArrayList<ActivityElement>();
		try {
			lst = (List<ActivityElement>) queryPageTempletElementPage(AppTaiHaoTempletConstant.HAITAO_APP,AppTaiHaoTempletConstant.HAITAO_APP_TOPICLIST, pagestart,pagesize);
		} catch (Exception e) {
			logger.error("APP海淘首页-专场列表查询报错", e);
			e.printStackTrace();
		}
