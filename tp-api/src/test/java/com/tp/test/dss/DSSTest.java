...
//		query.setShop("15800381241");
	...
	@Test
	public void testQueryChildPromoters() throws Exception {
		String startTime = "2012-12-06 ";  
		String endTime="2017-12-06 "; 
		
		
		QueryPromoter query = new QueryPromoter();
		query.setToken("dea30e485d4cead4893b922612f912d0");
		query.setType("2");
		query.setCurpage("1");
		query.setPagesize(10);
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd ");
			Date createBeginTime = sdf.parse(startTime);
			Date createEndTime = sdf.parse(endTime);
		...
		String json = JsonUtil.convertObjToStr(query);
		mockMvc.perform((post("/promoter/queryChildPromoters.htm").accept(MediaType.APPLICATION_JSON)
				.contentType(MediaType.APPLICATION_JSON).content(json.getBytes()))).andExpect(status().isOk())
				.andDo(print());
	}
}
