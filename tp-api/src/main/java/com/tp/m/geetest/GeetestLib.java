...
public class GeetestLib {
	...
	protected final String apiUrl = "https://api.geetest.com"; //极验验证API URL
	...
	private int registerChallenge() {
		try {
			...
			logger.info("GEETEST_REGISTER:GET_URL="+GET_URL);
			String result_str = readContentFromGet(GET_URL);
			gtlog("register_result:" + result_str);
			logger.info("GEETEST_REGISTER:REGISTER_RESULT="+result_str);
			if (32 == result_str.length()) {
				...
				logger.error("GEETEST_REGISTER_FAILED");
				return 0;
			}
		} catch (Exception e) {
			logger.error("GEETEST_REGISTER_ERROR.",e);
