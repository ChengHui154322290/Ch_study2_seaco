...
import com.tp.m.convert.WXMessageConvert;
import com.tp.m.helper.WXMessageHelper;
import com.tp.m.helper.cache.ActivityCacheHelper;
	...
	private String handleChannelPromoter(String eventKey,String fromUserName,String ip){
		if(StringUtil.equals(getPrefix(eventKey), QrcodeConstant.QRCODE_CHANNEL)){ //普通线下推广
			...
			if(null != pi){
				...
				userTO.setIp(ip);
				MemberInfoDto  memberInfoDto=memberInfoProxy.unionLogin(UserConvert.convertMemCallDto(userTO));
				 //默认扫码开通扫码功能
//				 userTO.setUserid(memberInfoDto.getUid());
