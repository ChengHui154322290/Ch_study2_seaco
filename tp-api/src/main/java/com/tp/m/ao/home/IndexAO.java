...
import com.tp.m.convert.IndexConvert;
...
public class IndexAO {
	...
	public MResultVO<Page<TopicVO>> getTodayNew(QueryIndex indexQuery) {
		try {
			...
			if(null != singleinfo)return new MResultVO<>(MResultInfo.SUCCESS,IndexConvert.convertIndexTopic(singleinfo,true));
			return new MResultVO<>(MResultInfo.SUCCESS);
		} catch (Exception e) {
...
//				singleinfo = hitaoAppProxy.getHaitaoAppService().htTopicAllListForDss(promoterId, StringUtil.getCurpageDefault(indexQuery.getCurpage()), PageConstant.DEFAULT_PAGESIZE_PROMOTER);								
				...
				singleinfo = hitaoAppProxy.getHaitaoAppService().htTopicAllList(null, StringUtil.getCurpageDefault(indexQuery.getCurpage()), PageConstant.DEFAULT_PAGESIZE,new String[0]);
			}
						
			if(null != singleinfo){
				if(promoterId != null){
					return new MResultVO<>(MResultInfo.SUCCESS,IndexConvert.convertTopicAndSingleForDss(promoterId, singleinfo,shareUrl,true) );
				}else{
					return new MResultVO<>(MResultInfo.SUCCESS,IndexConvert.convertTopicAndSingle( singleinfo,shareUrl,true));
				}
			}
			return new MResultVO<>(MResultInfo.SUCCESS);
		} catch (Exception e) {
	...
	public MResultVO<IndexModuleVO> queryIndexModular(QueryIndex indexQuery){
		try{
			...
			//功能标签模块
			vo.setLabs(IndexConvert.convertLabs(hitaoAppProxy.funLab()));
