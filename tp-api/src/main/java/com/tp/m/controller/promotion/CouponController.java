...
import com.tp.m.ao.promotion.CouponAO;
...
public class CouponController {
	...
	public String receive(HttpServletRequest request){
		try{
			...
			MResultVO<CouponUser> result = couponAO.receive(coupon);
			if(log.isInfoEnabled()){
	...
	public String couponList(HttpServletRequest request){
		...
		try{
			...
			for(int i=0,len=couponVOList.size();i<len;i++){
				...
				if("1".equals(couponVO.getExchangeXgMoney())){//允许兑换西客币的排在前面
	...
	@RequestMapping(value="/receiveCenterlist", method=RequestMethod.POST)
	@ResponseBody
	public String receiveCenterlist(HttpServletRequest request){
		String optName = "";
		try{
			String jsonStr = RequestHelper.getJsonStrByIO(request);
			Coupon coupon = (Coupon) JsonUtil.getObjectByJsonStr(jsonStr, Coupon.class);
			coupon.setIsShowReceive(CouponConstant.COUPON_SHOW_CENTER);//卡券领取中心显示
			coupon.setStatus(CouponStatus.PASSED.ordinal());//审核通过
			coupon.setExchangeXgMoney("1");//不兑换西客币
			coupon.setCouponReleaseEtime(new Date());
			 MResultVO<Page<CouponVO>> pageCoupon = couponAO.receiveCenterlist(coupon);
			if(log.isInfoEnabled()){
				log.info("[API接口 -{}优惠券列表 返回值] = {}",optName,JsonUtil.convertObjToStr(pageCoupon));
			}
			return JsonUtil.convertObjToStr(pageCoupon);
		}catch(MobileException me){
			log.error("[API接口 - {}优惠券列表  MobileException] = {}",optName,me.getMessage());
			log.error("[API接口 - {}优惠券列表  返回值] = {}",optName,JsonUtil.convertObjToStr(new MResultVO<>(me)));
			return JsonUtil.convertObjToStr(new MResultVO<>(me));
