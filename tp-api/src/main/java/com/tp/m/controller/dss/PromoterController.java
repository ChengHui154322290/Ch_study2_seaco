...
//			}
	...
	public String withdraw(HttpServletRequest request){
		try{
			...
			Long promoterId = promoterAO.authPromoter(promoter.getToken(), Integer.valueOf(promoter.getType()));
			promoter.setPromoterid(promoterId);
			promoter.setWithdrawType(DssConstant.PAYMENT_MODE.POINT.code);
...
//			MResultVO<MResultInfo> result = promoterAO.withdrawApply2(promoter);
	 ...
	 * 分销员查询
	 */
	@RequestMapping(value = "/queryChildPromoters",method = RequestMethod.POST)
	@ResponseBody
	public String queryChildPromoters(HttpServletRequest request){
		try{
			String jsonStr = RequestHelper.getJsonStrByIO(request);
			QueryPromoter promoter = (QueryPromoter) JsonUtil.getObjectByJsonStr(jsonStr, QueryPromoter.class);
			if(log.isInfoEnabled()){
			...
			AssertUtil.notBlank(promoter.getType(), MResultInfo.PROMOTER_TYPE_NULL);
			Long promoterId = promoterAO.authPromoter(promoter.getToken(), Integer.valueOf(promoter.getType()));
			promoter.setTopPromoterId(promoterId);
			MResultVO<Page<PromoterInfoVO>> result = promoterAO.queryChildPromoters(promoter);
			if(log.isInfoEnabled()){
				log.info("[API接口 -推广员分销员查询 返回值] = {}",JsonUtil.convertObjToStr(result));
			}
			return JsonUtil.convertObjToStr(result);
