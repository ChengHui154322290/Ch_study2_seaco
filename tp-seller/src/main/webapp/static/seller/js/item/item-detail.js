...
$(function(){
	...
	$('#backBtn').on('click',function(){
		newTab("tab4","商品管理列表","/seller/item/list.htm");
	});
	//商品对于供应商
	$('#selectSupplierbtn').on('click',function(){
		  ...
		  pageii=$.layer({
            ...
            area: ['800px', 500],                     
            iframe: {
                src : domain+'/seller/item/selectSupplier.htm?hasXgSeller='+hasXgSeller
            } 
        });
	}); 
	 ...
	 * 全选/取消全选
	 */
	$('#chooseItemStatus').on('click',function() {
	...
	//作废sku
	$('.cancelSkuBtn').on('click',function(){
	...
	//商品属性
	$('#selectAttributesbtn').on('click',function(){
		  pageii=$.layer({
            type : 2,
            title: '查询供应商列表',
            shadeClose: true,
            ...
            area: ['800px', 500],                     
            iframe: {
                src : domain+'/seller/item/selectAttribute.htm'
            } 
        });
	}); 
	
	//查询自销商品的sku对应的供应商
	$('.querySkuSupplier').on('click',function(){
		var skuId = $(this).attr('id');
		pageii=$.layer({
            type : 2,
            title: '查询Sku对应的供应商列表',
            ...
            area: ['800px', 500],                     
            iframe: {
                src : domain+'/seller/item/getSkuSupplier.htm?skuId='+skuId
            } 
        });
	}); 
	...
	//查询自销商品的sku对应的供应商
	$('.querySkuArtNumber').on('click',function(){
		var skuId = $(this).attr('id');
		pageii=$.layer({
            type : 2,
            title: '设置sku备案信息', 
            ...
            area: ['1500px', 700],                     
            iframe: { 
                src : domain+'/seller/item/getSkuArtNumber.htm?skuId='+skuId
            } 
        });
	}); 
	...
	$('#addDummyAttributeBtn').on('click',function(){
		//添加自定义属性到商品中
	...
	$('#inputFormSaveBtn').on('click',function(){
		...
		if(validateDetailForm()){
				$.ajax({ 
					url: '/seller/item/saveDetail.htm', 
					dataType: 'json',
					...
					success: function(resultInfo) {
						if(resultInfo.success){//成功
							 newTab("item_sku","编辑sku信息","/seller/item/detail.htm?detailId="+resultInfo.data);
							layer.alert('操作成功',8);
					    }else{//失败
...
var appendToSuppList = function(selectSeller,supplierId,supplierName,supplierType,supplierTypeName){
	/*
	 自营(西客商城) ： 代销 sell ,联营 Associate
         商家：            自营  Purchase
	*/
	var __supplierName = "西客商城";
	var __sellerName="西客商城";
	var __supplierId = 0;
	var saleType = 0;
	if(supplierType=="Associate"){
		saleType =1 ;
...
function bindSkuListData(){
	...
	$('.skuList').each(function(){
		...
		if(!skuId.isEmpty()){
			...
			if(status != 2){
				if (statusCheckbox.is(':checked')==true){
					status = 1 ;
				}else{
					status = 0;
				}	
			...
			if(status != 2){
				...
				var statusCheckbox = $(this).find('[name=status]');
				if (statusCheckbox.attr('checked')){
					status = 1 ;
				}else{
					status = 0;
...
function validateDetailForm(){
	...
	if(unit == ""){
	  ...
	  var  weightNet =  $("#weightNet").val();
	  if(weightNet==null||weightNet==""){
		  layer.alert("净重不能为空"); 
		  return false;
	  }
	...
	if ($("#statusCheckbox").get(0).checked  && skuImageCount<3 ){
		alert("上架Sku的图片必须为3-10张");
		return false ;
	}
