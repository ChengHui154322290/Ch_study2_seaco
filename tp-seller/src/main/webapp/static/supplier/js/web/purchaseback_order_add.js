jQuery(function(){
		...
		$.ajax({
			...
			success:function(resultInfo){
				if(resultInfo.success){
					...
					jQuery("#orderAddSave").attr("disabled",true);
					jQuery("#orderAddSub").attr("disabled",true);
					window.location.href=domain+'/supplier/purchaseorderbackList';
				}else{
	...
	jQuery("#orderAddSub").click(function(){
		...
		//jQuery("#order_form").attr("action", domain + "/supplier/purchaseOrderBackSub.htm");
		//jQuery("#order_form").submit();
		//jQuery("#orderAddSave").attr("disabled",true);
		//jQuery("#orderAddSub").attr("disabled",true);
		var form = jQuery("#order_form");
		$.ajax({
			url:domain+"/supplier/purchaseOrderBackSub.htm",
			data:form.serialize(),
			dataType:'json',
			type:'post',
			success:function(resultInfo){
	...
	jQuery("#quotationShowAudit").click(function(){
		...
		ajaxRequest({
			method:'post',
			url:'/supplier/purchaseOrderBackExam/auditPage.htm',
			data:{"quoId":quoId},
	...
	jQuery("#quotationShowCancel").click(function(){
		confirmBox('确定取消？',function(){
			...
			jQuery("#quotationShowCancel").attr("action","/supplier/purchaseOrderBackCancel.htm");
			jQuery("#order_form").submit();
			jQuery("#orderAddSave").attr("disabled",true);
			jQuery("#orderAddSub").attr("disabled",true);
		});
...
function addOrderItem(){
	...
	// by zhs 01171412 增加 仓库id
	var warehouseId = jQuery("#pur_order_add_warehouse_id_hidden").val();
	...
	ajaxRequest({
		...
		url:'/supplier/purchaseorderback/getOrderItemInfoForm.htm',
		success:function(data){
