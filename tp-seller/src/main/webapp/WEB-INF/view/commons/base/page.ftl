...
<nav>
    ...
    <#-- 前组-->  
    <#if offset gt 0>  
        <li><a href="javascript:void(0)" onclick="_gotoPage(${offset * maxsteps})">…</a></li>
    <#else>  
        <#--
    ...
    <#if ipage gt 0 && ipage lte totalpage>  
        <#list (offset * maxsteps + 1)..pagelist as num>  
            <#if ipage != num>  
                <li><a href="javascript:void(0)" onclick="_gotoPage(${num})">${num}</a></li>
            <#else>  
    ...
    <#if offset lt offsetLast>  
        <li><a href="javascript:void(0)" onclick="_gotoPage(${(offset + 1) * maxsteps + 1})">…</a></li>
    </#if>  
    <#-- 尾页 -->  
