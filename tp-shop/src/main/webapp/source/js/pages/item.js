...
$(function() {
	var tel = xigou.getQueryString("tel");
	if(null!=tel && ''!=tel){
		xigou.setSessionStorage('tel',tel);
	}
	Initkf();
...
//		var tel=xigou.getLocalStorage('login_name');
		var link = 'item.htm?tid=' + tid + '&sku=' + specialSku + '&tel='+ xigou.getQueryString("tel");
		if (shop) {
			link += '&shop=' + shop;
		}
	...
	$(".fee")[xigou.events.click](function(){
		$("#mask").show();
		$("#taxationModal").show();
...
//		var dssUser =JSON.parse(xigou.getSessionStorage("dssUser"));
//		var shopMobile = dssUser ?(dssUser.mobile && dssUser.token ?dssUser.mobile :null):null;
//	}
	if(xigou.getLocalStorage("shop")){
		shopMobile = xigou.getSessionStorage("shop")
	}
	var params = {
...
wx.ready(function () {
	...
	var sku = xigou.getQueryString('sku');
	var tel = xigou.getQueryString("tel");
	lineLink = lineLink + '?tid=' + tid + '&sku='+sku+'&tel='+tel;
	if(shopOwer != null) {
...
//初始化客服
function Initkf(){
	var name = '[' + xigou.getSessionStorage("eshopName") + ']' + (xigou.getLocalStorage("show_name")|| xigou.getLocalStorage("login_name") || "会员");
	if (name) {
	...
	$('body').append('<div class="kt-btn" id="header_img2" onclick="ysf.open();return false;"></div><div class="header_imgs header_img2" id="header_img2" style="display:none;" onclick="ysf.open();return false;"></div>');
...
$('.group-back')[xigou.events.click](function(){
	if(history.go(-1) == null){
		$(".group-back").attr("href", "index.html");
	}
});

/*$('.goBack')[xigou.events.click](function(){
	if($(".goBack").attr("href") == "" || $(".goBack").attr("href") == null || $(".goBack").attr("href") == undefined){
