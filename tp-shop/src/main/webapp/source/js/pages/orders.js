	...
	var o = data,
		...
		for(var i=0; i<length; i++){
			...
			switch(alllist[i].status){
				...
				break;
				case '2':  //待支付
						html.push('<a class="bottom_btn orders_btn_pay" price="'+alllist[i].orderprice +'"  ordercode="'+alllist[i].ordercode+'">立即支付</a> ');
...
function ordersgotoPay() {
	...
	$('.orders_btn_pay')[xigou.events.click](function(e) {
		...
		if(isWeiXin()){
			...
			var param= payid+","+ ordercode+","+price+","+xigou.getToken()+","+xigou.channelcode+","+host;
