var userMobile = "",
 	oDom = '<div class="li-div bd_btm">'+
		...
		'</div>'+
		'</div>';
var imgUrl = "http://"+window.location.host+"/dss/img/"+xigou.channelcode+"_share_icon.png";
$(function() {
	$.ajax({
	    url : imgUrl,
	    async : false,
	    type : 'HEAD',
	    error : function() {
	    	imgUrl="http://"+window.location.host+"/dss/img/share_icon.png";
	    }, success : function() {
	    }
	});  
	getDssUserInfo();
	if (isWeiXin()) {
		...
		wx.ready(function () {
			...
			var title = '快来加入' + eshopName + '轻创业平台吧！';
			var desc =  eshopName + '，指尖上的跨境微店！0囤货 0风险 0压力，汇聚全球精品，全程溯源，内测期间超低门槛入驻！全球购，为圆梦而生，动动手指即可拥有自己的跨境微店。';
					...
					callback: function(response, status) {
						if (status == xigou.dictionary.success) {
							if(response != null && response != "undefined"){
								...
								$(".div_dsslogo").html("<img class='div_dsslogo_img' src='data:image/png;base64,"+response.img+"'/>");
								...
								if(!isWeiXin()){
									$(".nowx_tip").html("你也可以手动复制链接：<br/>http://"+xigou.channelcode+".shop.51seaco.com/dssinvite.html?i=" + userMobile);
								}
							}
							else {
