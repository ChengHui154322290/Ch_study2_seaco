...
var shop = xigou.getQueryString('shop'); //分享店铺
//记录锚点,定位免税店首页
...
$(function() {
	...
	if (isWeiXin()) {
		...
		if (channelinfo){
			var data = JSON.parse(channelinfo);
	...
	if(shop){
		xigou.setSessionStorage('productdetails_backurl', 'hd.htm?tid=' + specialid+'&shop='+shop+'&tel='+xigou.getLocalStorage('login_name'));
	}else{
		xigou.setSessionStorage('productdetails_backurl', 'hd.htm?tid=' + specialid+'&tel='+tel);
	}
						...
						if (json.code == 0) {
							...
							else
							{
								for (var i = 0; i < data.length; i++) {
									...
									if(shop){
										htmlData.push('		<a href="item.htm?tid=' + tid +'&sku=' + sku + '&shop='+shop+'">');
									}else{
