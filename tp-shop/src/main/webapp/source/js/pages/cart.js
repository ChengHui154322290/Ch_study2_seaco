...
var shopId = xigou.getLocalStorage('promoterid');
$(function () {
...
function shopcart() {
	...
	$shopcart[xigou.events.click](function() {
		//获取当前选中的所有商品
		var $shopCartCheck = $('.cart-list input:checked'),
						...
						if (status == xigou.dictionary.success) {
							...
							if (json.code == 0) {
								...
								xigou.removeClearingData();
									window.location.href = "settle.html?from=cart";
							} else {
								var dia=$.dialog({
									title:'',
									content: json.msg ,
...
function GroupBackFun(json) {
	...
	for (var i = 0; i < json.mergeitems.length; i++) {
		...
		for (var j = 0; j < mergeitemsItem.products.length; j++) {
			...
			var itemhref = 'item.htm?sku='+ item.sku + '&tid=' + item.tid + '&url=cart.html';
			...
			htmlData.push('			<div class="cart_product_spe">' + desc + '</div>');
			htmlData.push('			<div>');
	...
	for (var i = 0; i < json.invaliditems.length; i++) {
		...
		var Yuan = invalidItems.lineprice.split('.')[0] || '00';
		var Jiao = invalidItems.lineprice.split('.')[1] || '00';
		var itemhref = 'item.htm?sku='+ invalidItems.sku + '&tid=' + invalidItems.tid + '&url=cart.html';
		...
		htmlInvalid.push('		</a>');
		htmlInvalid.push('		<div class="ui-list-img-gray2">' + invalidItems.statusdesc + '</div>');
		...
		htmlInvalid.push('		</div>');
		var desc = '';
		if (invalidItems.taxrate) {
			desc = '税率:' + parseInt(invalidItems.taxrate) + '% ';
		}
		if (invalidItems.limitcount) {
			desc += '限购:' + solveNull(invalidItems.limitcount, "") + '件';
		}
		htmlInvalid.push('		<div class="cart_product_spe">' + desc + '</div>');
		htmlInvalid.push('		<div>');
	...
	if (json.mergeitems.length != 0) {	// 有效商品不为空
		$("#cart-list").append(htmlData.join(" "));
		$("#cart-list").show();
		$(".cartnull").hide();
	}

	if (json.invaliditems.length != 0) {
		$(".cartnull").hide();
		$('.car_invalid').show();
		$(".warm_tip").hide();
		$(".split_bg").hide();
...
function changeNum(mergeitems, params) {
	...
	$.each(taxesInfo, function(index, mergeitemsindex) {
		...
		var divChannelItem = $($('#cart-list li.div_channel')[index]);
...
function selectAll(type) {
	...
	var checkBox = document.getElementById("cart-list").getElementsByTagName('input');
