...
import com.tp.shop.convert.AfterSalesConvert;
...
public class AfterSalesAO {
	...
	public MResultVO<Page<AfterSalesVO>> list2Detail(QueryAfterSales afterSales) {
		try {
			ResultInfo<PageInfo<RejectInfo>> result = rejectInfoProxy.queryMobilePageListByRejectQuery(afterSales.getUserid(), StringUtil.getCurpageDefault(afterSales.getCurpage()), PageConstant.DEFAULT_PAGESIZE,null,afterSales.getChannelcode());
