...
import com.tp.shop.convert.CouponConvert;
...
public class CouponAO {
	...
	public MResultVO<Page<CouponVO>> queryCouponListByOrder(QueryCoupon coupon){
		...
		ResultInfo<Map<String, List<OrderCouponDTO>>> res = orderCouponProxy.queryOrderCoupon(coupon.getUserid(), coupon.getUuid(), RequestHelper.getPlatformByName(coupon.getApptype()).code,promoterAO.getPromoterIdByChannelCode(coupon.getChannelcode()));
		if(res.isSuccess()){
	...
	public MResultVO<Integer> queryOrderCouponCount(QueryCoupon coupon){
		//Map<String, List<CouponDTO>>key=优惠券类型 0 : 满减券 1：现金券 ，value=用户的优惠券列表 CartConstant.TYPE_SEA
		ResultInfo<Integer> res = orderCouponProxy.queryOrderCouponCount(coupon.getUserid(), coupon.getUuid(), RequestHelper.getPlatformByName(coupon.getApptype()).code,promoterAO.getPromoterIdByChannelCode(coupon.getChannelcode()));
		if(res.isSuccess()){
