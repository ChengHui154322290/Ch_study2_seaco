...
import com.tp.shop.convert.CartConvert;
import com.tp.shop.convert.ShoppingCartConvert;
...
public class CartAO {
	...
	public MResultVO<MResultInfo> add(QueryCart cart){
		try{
			...
			cart.setShopId(promoterAO.getPromoterIdByChannelCode(cart.getChannelcode()));
	 ...
	 * @return
	 */
	public MResultVO<CartSupVO> supCount(Long userid,String channelCode){
		try{
			ResultInfo<Integer> count = cartProxy.queryQuantityCountByMemberId(userid,promoterAO.getPromoterIdByChannelCode(channelCode));
			if(count.isSuccess()){
	...
	public MResultVO<CartVO> load(QueryCart cart) {
		try {
			...
			ResultInfo<ShoppingCartDto> cartDto = cartProxy.queryCartByMemberId(cart.getUserid(), cart.getIp(), StringUtil.getLongByStr(cart.getRegcode()), PlatformEnum.getCodeByName(cart.getApptype()), cart.getToken(),promoterAO.getPromoterIdByChannelCode(cart.getChannelcode()));
	...
	public MResultVO<CartVO> operation(QueryCart cart,String operationName) {
		try {
			...
			Long shopId = promoterAO.getPromoterIdByChannelCode(cart.getChannelcode());
			...
			if(cart.getType().equals(CartEnum.CheckType.CHECK_ALL.code)){//全选
				result = cartProxy.selectAllCartItem(cart.getUserid(),cart.getToken(),Boolean.TRUE,shopId);
			}else if(cart.getType().equals(CartEnum.CheckType.CHECK_CANCEL_ALL.code)){//取消全选
				result = cartProxy.selectAllCartItem(cart.getUserid(),cart.getToken(),Boolean.FALSE,shopId);
				...
				result = cartProxy.operateTotal(cart.getUserid(), cart.getToken(),shopId);
			}
			if(result.isSuccess()){
