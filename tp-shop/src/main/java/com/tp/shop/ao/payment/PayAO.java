package com.tp.shop.ao.payment;
...
public class PayAO {
	...
	public MResultVO<BasePayVO> paymentOrder(QueryPay pay){
		try{
			...
			AppPayData payData = appPaymentProxy.getAppData(PayConvert.convertPayQuery(pay));
	...
	public MResultVO<PayResultVO> paymentResult(QueryPay pay){
		try{
			...
			orderRedeemItem.setOrderCode(Long.valueOf(pay.getOrdercode()));
			ResultInfo<List<OrderRedeemItem>> orderRedeemItemList=orderRedeemItemProxy.queryByObject(orderRedeemItem);
			return new MResultVO<>(MResultInfo.SUCCESS,PayConvert.convertPayResult(info,orderRedeemItemList.getData()));
		}catch(MobileException ex){
