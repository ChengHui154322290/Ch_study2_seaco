...
import com.tp.shop.convert.SearchConvert;
...
public class SearchAO {
    ...
    public MResultVO<PageForSearch<SearchItemVO, List<SearchShopVO>>> search(QuerySearch search) {
        try {
            ...
            ResultInfo<ShopResult> shopResult = searchProxy.searchShop(query);
            Long topicId = getCorrespondingTopicIdBySupplierId(shopResult);
            if (result.isSuccess())
                return new MResultVO<>(MResultInfo.SUCCESS, SearchConvert.convertSearchItem(result.getData(), shopResult.getData(), topicId));
