package com.tp.shop.controller.order;
...
import com.tp.shop.ao.order.OrderAO;
...
public class OrderController {
	...
	public String getSubmitInfo(HttpServletRequest request){
		try{
			...
			Long promoterId = promoterAO.getPromoterIdByChannelCode(order.getChannelcode());
			order.setShopId(promoterId);
			...
			order.setPointType(order.getChannelcode());
...
//			/*替换为WeixinExternalPay test end*/
	...
	public String detail(HttpServletRequest request){
		try{
			String jsonStr = RequestHelper.getJsonStrByIO(request);
			QueryOrder order = (QueryOrder) JsonUtil.getObjectByJsonStr(jsonStr, QueryOrder.class);
			order.setChannelcode(RequestHelper.getChannelCode(request));
			if(log.isInfoEnabled()){
