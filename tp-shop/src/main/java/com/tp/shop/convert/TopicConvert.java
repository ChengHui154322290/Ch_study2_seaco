...
public class TopicConvert {
	 ...
	 * @return
	 */
	public static TopicVO convertTopic(AppSingleInfoDTO single){
	 ...
	 * @return
	 */
	public static TopicVO convertTopicAndSingle(AppSingleAllInfoDTO single,String shareUrl){
		TopicVO t = new TopicVO();
		t.setTid(StringUtil.getStrByObj(single.getSpecialid()));
		...
		if(single.isSingle()){
			...
			if(CollectionUtils.isNotEmpty(topicItemList)){
				...
				for(Products topicItem : topicItemList){
					...
					if(StringUtil.isNotBlank(topicItem.getType()))vo.setStatus(getStatusByPrdType(topicItem.getType()).code);
					itemlist.add(vo);
				}
	 ...
	 * @return
	 */
	public static TopicVO convertTopicAndSingleForDSS(AppSingleAllInfoDTO single,String shareUrl){
		TopicVO t = new TopicVO();
		t.setTid(StringUtil.getStrByObj(single.getSpecialid()));
		t.setName(single.getSpecialName());
		t.setImgurl(ImgHelper.getImgUrl(single.getImageurl().get(0),ImgEnum.Width.WIDTH_750));
		...
		//封装单品团信息
		if(single.isSingle()){
			t.setImgurl(ImgHelper.getImgUrl(single.getImageurl().get(0),ImgEnum.Width.WIDTH_346));
//			t.setName( " " );
			t.setChannel(single.getChannelName());
			t.setCountryimg(ImgHelper.getImgUrl(single.getCountryImageUrl(), ImgEnum.Width.WIDTH_30));
			t.setCountryname(single.getCountryName());
			...
			if(CollectionUtils.isNotEmpty(topicItemList)){
				...
				for(Products topicItem : topicItemList){
					ProductVO vo = new ProductVO();
					vo.setName(topicItem.getName());
					vo.setImgurl(ImgHelper.getImgUrl(topicItem.getImgsrc(), ImgEnum.Width.WIDTH_296));
					vo.setOldprice(NumberUtil.sfwr(topicItem.getLastValue()));
					vo.setPrice(NumberUtil.sfwr(topicItem.getNowValue()));
					vo.setDiscount(NumberUtil.calDiscount(StringUtil.getDoubleByStr(vo.getPrice()), StringUtil.getDoubleByStr(vo.getOldprice())));
					vo.setTid(topicItem.getTopicid());
	 ...
	 * @return
	 */
	public static TopicDetailVO convertTopicDetail(com.tp.model.mmp.Topic topic,String shareUrl,String channelCode){
		...
		if (null != topic) {
			...
			if(StringUtils.isNotBlank(t.getTophtml())){
				t.setTophtml(t.getTophtml().replaceAll("m.51seaco.com", channelCode+".shop.51seaco.com"));
			}
			t.setShareurl(shareUrl);
		}
