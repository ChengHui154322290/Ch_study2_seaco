...
public class IndexConvert {
	 ...
	 * @return
	 */
	public static Page<TopicVO> convertIndexTopic(PageInfo<AppSingleInfoDTO> singleinfo){
		...
		if(null != infos){
			if(CollectionUtils.isNotEmpty(infos)){
				for(AppSingleInfoDTO single : infos){
					...
					tlist.add(TopicConvert.convertTopic(single));
				}
	 ...
	 * @return
	 */
	public static Page<TopicVO> convertTopicAndSingle( PageInfo<AppSingleAllInfoDTO> singleinfo,String shareUrl){
		Page<TopicVO> pages = new Page<>();
		...
		if(null != infos){
			if(CollectionUtils.isNotEmpty(infos)){
				for(AppSingleAllInfoDTO single : infos){
					...
					tlist.add(TopicConvert.convertTopicAndSingle(single,sUrl));							
				}
				pages.setFieldTCount(tlist,singleinfo.getPage(), singleinfo.getRecords());
			}
			pages.setCurpage(singleinfo.getPage());
	 ...
	 * @return
	 */
	public static Page<TopicVO> convertTopicAndSingleForDss(Long promoterId, PageInfo<AppSingleAllInfoDTO> singleinfo,String shareUrl ){
		Page<TopicVO> pages = new Page<>();
		List<AppSingleAllInfoDTO> infos = singleinfo.getRows();
		List<TopicVO> tlist = new ArrayList<>();
		if(null != infos){
			if(CollectionUtils.isNotEmpty(infos)){
				for(AppSingleAllInfoDTO single : infos){
					...
					tlist.add(TopicConvert.convertTopicAndSingleForDSS(single,sUrl));													
				}
				pages.setFieldTCount(tlist,singleinfo.getPage(), singleinfo.getRecords());
			}
			pages.setCurpage(singleinfo.getPage());
