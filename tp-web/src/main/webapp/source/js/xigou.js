 ...
 */
var xigou = xigou || {};
this.getXgOpenIdUrl = 'http://m.seagoor.com/paymentbridgeForSeaCo.html?from=seaCo';
...
(function($) {
	...
	this.activeHost = "//"+window.location.host+"/";
	...
	this.activeUrl = {
		index: {
			...
			dc:"dc.htm",  //首页-活动模块
			advertlog:"advert/recordAdvert.htm",//记录广告日志
		},
		user: {
			logon: "user/logon.htm", //用户-登录
			getcaptcha: "user/getcaptcha.htm", //用户-获取手机验证码
			getcaptchaNew:"user/getcaptchaNew.htm",
			...
			init:"user/init.htm",//初始化用户（获取用户信息）
			modifymobile:"user/modifymobile.htm",//修改用户手机号
			checkmobile:"user/checkmobile.htm"//验证原手机号
		},
		dssuser: {
		...
		coupon:{
			...
			receiveCenterlist: "coupon/receiveCenterlist.htm",
			receiveCoupon: "coupon/receiveCoupon.htm",
		},
		point:{
			list:'point/list.htm'
		},
		...
		promoter: {
			...
			queryChildPromoters:"/promoter/queryChildPromoters.htm"
		},
		groupbuy: {
		...
		orderredeem:{
			list:'/order/redeem/list.htm',
			count:'/order/redeem/count.htm',
			paramskulist:'/order/redeem/param/skulist.htm',
			paramcodestatus:'/order/redeem/param/codestatus.htm',
			exchange:'/order/redeem/exchange.htm',
			userlist:'/order/redeem/user/list.htm',
			usersave:'/order/redeem/user/save.htm',
			userdelete:'/order/redeem/user/delete.htm'
		},
		offlinegb:{
		...
		usersign:{
			showcalendar:"member/sign/showcalendar.htm",
			show:"member/sign/show.htm"

		},
		seagoorpay:{
			code:"/seagoorpay/code.htm",
			querypaystatus:"/seagoorpay/querypaystatus.htm"
		},
		switches:{
			swt:"/swt.htm"
		}
	...
	this.publicParams = {
		get: function(urls) {
			...
			arry.push('scheme=' + window.location.protocol.replace(":",""));
...
//					xigou.alert("网络已断开");
	...
	this.recordAdverstLog = function(params) {
		params.requestBody = params.requestBody || {};
		var path = this.activeHost + this.activeUrl.index.advertlog;
		ajaxPost(params, path);
	}
...
(function($){
	...
	this.getwxOpenid = function(type, getNickName) {
		var url = '';
		if (type == 0) {
			url = '../getopenid.html?back_url=' + encodeURIComponent(location.href);
		}
		else {
			url = './getopenid.html?back_url=' + encodeURIComponent(location.href);
		}
	...
	var _this = this;
	this.getwxOpenidUnion = function(type, getNickName) {
		var url = '';
		if (type == 0) {
			url = '../getopenidunion.html?back_url=' + encodeURIComponent(location.href);
		}
		else {
			url = 'getopenidunion.html?back_url=' + encodeURIComponent(location.href);
		}
...
(function($) {
	...
	var create = function() {
		if ($("#cpic_ui_box").size() <= 0) {
			...
			h.push("    <div class='cpic_ui_loading_img'></div>");
			h.push("</div>");
			h = $(h.join(""));
			$("body").append(h);
		}
...
function onUnionBridgeReady(wx_param, pid, ordercode,sc, host,backurl){
		...
		WeixinJSBridge.invoke(
			'getBrandWCPayRequest', wx_param,function(res){
				...
				if(res.err_msg == "get_brand_wcpay_request:cancel" ) {
					...
					window.location.href = host + "orders.html";
					}
				}
				//用户支付失败
				if(res.err_msg == "get_brand_wcpay_request:fail" ) {
...
var channelcode  = xigou.getLocalStorage("channelcode");
if(yiqifarddate!=undefined && yiqifarddate!=null && yiqifarddate!=''){
