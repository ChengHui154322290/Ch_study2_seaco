...
$(function() {
	...
	if (from || isappinstalled) {
		...
		var link = 'item.html?tid=' + tid + '&sku=' + specialSku;
		if (shop) {
			link += '&shop=' + shop;
		}
...
$(window).scroll(function(){
	...
	if(scrolltop >=100){
		$(".div_show_detail")
		$(".detail-content").show();
		if ($(".div_show_detail").hasClass('up')) {
	...
	if(scrolltop >=222){
		$(".div_show_detail").removeClass('up').addClass('down');
	}else{
		$(".div_show_detail").addClass('up').removeClass('down');
	}
		...
		callback: function(response, status) { //回调函数
			if (status == xigou.dictionary.success) {
				...
				if (json.code == 0) {
					...
					var inventory = json.data.inventoryCount;
					//商品图片
