...
function pay_success(json) {
	...
	if(orderRedeemItemList.length>0){
		...
		for(var i = 0;i<orderRedeemItemList.length;i++){
			...
			html.push('<li><p><span class="redeem_name">'+orderRedeemItemList[i].redeemName+'</span><span class="redeem_code_nub">'+orderRedeemItemList[i].redeemCode+'<span qr_code="'+orderRedeemItemList[i].qrCode+'" class="redeem_qrcode" ></span></p></li>');
		}
