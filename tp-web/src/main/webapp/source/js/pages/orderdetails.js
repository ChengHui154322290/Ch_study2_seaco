...
function setHtml(json) {
	...
	if (data.ordertype != 8) {
		...
		$(".div_add_info").hide();
		if ((data.status == 5 || data.status == 6 || data.status == 0)
				&& orderRedeemItemList.length > 0) {
			$(".div_add_tel").hide();
			$(".coupon_code").show();
			...
			for (var i = 0; i < orderRedeemItemList.length; i++) {
				...
				if (orderRedeemItemList[i].redeemCodeState != "1") {
					html
							.push('<li><p><span class="redeem_name">'
									+ orderRedeemItemList[i].redeemName
									+ '</span><span class="redeem_code_nub" style="text-decoration:line-through">'
									+ orderRedeemItemList[i].redeemCode
									+ '</span></p></li>');
				} else {
					html.push('<li><p><span class="redeem_name">'
							+ orderRedeemItemList[i].redeemName
							+ '</span><span class="redeem_code_nub">'
							+ orderRedeemItemList[i].redeemCode
							+ '</span><span qr_code="'
							+ orderRedeemItemList[i].qrCode
							+ '" class="redeem_qrcode" ></span></p></li>');
				}
			}
			...
			$(".redeem_qrcode")[xigou.events.click](function() {
				...
				$(".redeem_qrcode_img").css("background-image",
						"url('data:image/png;base64," + ercode + "')")
	...
	for (var i = 0; i < itemsList.length; i++) {
		...
		// item.html?sku=07020700020101&amp;tid=12004
		if (data.ordertype != 8) {
			var href = 'item.html?sku=' + item.sku + '&tid=' + item.tid;
		} else {
		...
		htmlData.push('		<span class="item_price">'
				+ createPriceHTML(item.price, "item_price_yuan") + '</span>');
		...
		if (item.specs.length > 0) {
			htmlData
					.push('		<div class="item_count">×' + item.count + '</div>');
			htmlData.push('			<div style="width: 185px;">' + item.specs
					+ '</div>');
		} else {
			htmlData.push('		<div class="item_count1">×' + item.count
					+ '</div>');
		}
		htmlData.push('		</div>');
		htmlData.push('		<div class="item_lineprice">'
				+ createPriceHTML(item.lineprice, "item_lineprice_yuan")
				+ '</div>');
		if (item.isreturn == 0 && data.status == 6 && data.ordertype != 8) {
			htmlData.push('		<div class="item_aftersale" index = "' + i
					+ '">申请售后</div>');
		}
		htmlData.push('	</div>');
		htmlData.push('</div>');
	...
	$('.totle_price_fen').html(
			createPriceHTML(data.baseprice, 'totle_price_yuan')); // 应付款

	// 底部按钮
	var htmlButtomData = [];
	// statusCN = status == '0'? '已取消':status == '2'? '待支付':status == '4'?
	// '已付款':status == '5'? '已发货':status == '6'? '已收货':"",
	switch (data.status) {
	case "0": // 已取消
		htmlButtomData.push('<a class="bottom_btn orders_btn_del" ordercode="'
				+ data.ordercode + '" payid="' + data.payid + '">删除订单</a>');
		break;
	case "2": // 待支付
		htmlButtomData.push('<a class="bottom_btn detail_btn_pay" ordercode="'
				+ data.ordercode + '" payid="' + data.payid + '">立即支付</a>');
		htmlButtomData
				.push('<a class="bottom_btn detail_btn_pay_cancal" ordercode="'
						+ data.ordercode + '" payid="' + data.payid
						+ '">取消订单</a>');
		break;
	case "4": // 已付款
	...
	case '5': // 已发货
		if (data.ordertype != 8) {
			htmlButtomData
					.push('<a class="bottom_btn orders_btn_pay_look" ordercode="'
							+ data.ordercode + '">订单追踪</a> ');
			htmlButtomData
					.push('<a class="bottom_btn orders_btn_pay_sure" ordercode="'
							+ data.ordercode + '">确认收货</a> ');
		} else {
			if (isOverDue == false) {
				htmlButtomData
						.push('<a class="bottom_btn detail_btn_pay_cancal" ordercode="'
				...
				htmlButtomData
						.push('<a class="bottom_btn orders_btn_del" ordercode="'
								+ data.ordercode
								+ '" payid="'
								+ data.payid
								+ '">删除订单</a>');
...
function detailgotoPay() {
	$('.detail_btn_pay')[xigou.events.click]
			(function(e) {
								...
								if (status == xigou.dictionary.success) {
									if (null == response) {
										...
										case "0":
											...
											break;
										case "-1003":
											window.location.href = "payresult.html?pid="
													+ payid
													+ "&ordercode="
													+ ordercode;
											break;
					...
					callback : function(response, status) { // 回调函数
						if (status == xigou.dictionary.success) {
							if (response.code == 0) {
								...
								setTimeout(function() {
									var url = "orders.html";
									var nDefaulePage = xigou
											.getQueryString("defaule");
									if (nDefaulePage) {
...
// 初始化客服
function Initkf() {
	var name = xigou.getLocalStorage("show_name")
			|| xigou.getLocalStorage("login_name") || "西客会员";
	if (name) {
		var oScript = document.createElement("script");
		oScript.type = "text/javascript";
		oScript.src = 'https://qiyukf.com/script/6e39dddabff63d902f55df3961c2793d.js?name='
				+ name + '&mobile=' + name;
		$('body')[0].appendChild(oScript);
	}

	$('div.coupon_code').append(
			'<div class="kf_btn" onclick="ysf.open();return false;"></div>');
}