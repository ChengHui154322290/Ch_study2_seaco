	...
	$.each(dataList, function(index, data){
		...
		if (1 == data.type) {
			...
			if (dssUserMobile && data.commision && showFlag) {	// 返佣
				...
				htmlData.push('<div class="old_price">返佣：<span class="return">¥' + data.commision + '</span></div>');
			}
			else {
		...
		// 主题团
			...
			htmlData.push('<a href="hd.html?tid=' + data.tid + '&from=shop" class="hot_link"">');
			...
			if(data.itemlist && data.itemlist.length > 0 ){
				...
				for(var j = 0;j<oList.length;j++){
					...
					htmlData.push( '<li style = "width:'+oWidth+'px"><a style="display:block;width:100%;height:'+oWidth+'px;overflow: hidden;" href="item.html?tid='+oList[j].tid+'&sku='+oList[j].sku+'"><img src = '+oList[j].imgurl+' /></a>');
				...
				htmlData.push('<li><a href="hd.html?tid=' + data.tid + '&from=shop"><img src = "img/zt_more.png" /></a></li>');
				htmlData.push('</ul>');
				htmlData.push('</div>');
			}
		}
