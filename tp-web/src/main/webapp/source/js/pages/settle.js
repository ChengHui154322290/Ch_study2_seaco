	...
	xigou.getLoginUserInfo({
		callback: function(userinfo, status) {
			if (status == xigou.dictionary.success) {
				...
				if(!uuid && xigou.getQueryString("selectType") != "2") {
...
function initPageData(data) {
	...
	if (data) {
		...
		'<div class="floor_product_title item_info">' +
			...
			'</div>' +
			'<div class="order_size_count">' +
				'<div class="item_count">#count#</div>' +
				'<div style="margin-right: 65px;">#itemspecs#</div>' +
			'</div>' +
			'<div class="item_count_price">#countPrice#</div>' +
		'</div>' +
	'</li>',
htmls = [];
var template1 = '<li class="ui-border-t clearfix">' +
		'<div class="ui-list-img floor_product_img"><img class="settle-product-icon" src="#img#" specialid="#specialid#" productid="#productid#"></div>' +
		'<div class="floor_product_title item_info">' +
			'<div  class="cart_product_name">'+
				'<div class="item_price">#price#</div>'+
			...
			'</div>' +
			'<div class="order_size_count">' +
				'<div class="item_count1">#count#</div>' +
			'</div>' +
			'<div class="item_count_price">#countPrice#</div>' +
		'</div>' +
	'</li>',
...
var shopName = xigou.getSessionStorage("ShopInfoName");
for(var j = 0,jlen = productinfo.length; j < jlen; j++){
		var pInfo = productinfo[j];
		htmls.push('<div class="floor_product">');
		var itemType= xigou.getSessionStorage("itemType");
		if(itemType =="2"){
			htmls.push('<div class="floor_product_shopName"><span class="floor_product_name">'+shopName+'</span></div>')
		}else{
			htmls.push('<div class="floor_product_tap"><span>'+pInfo.channel+'</span></div>');
		}
		...
		for(var n = 0; n < pInfo.products.length; n++){
			...
			if(pItem.itemspecs && pItem.itemspecs != ""){
				htmls.push(
						...
						.replace("#itemspecs#", pItem.itemspecs)
					);
			}else{
				htmls.push(
						template1.replace("#img#", pItem.imgurl)
						.replace("#specialid#", pItem.tid)
						.replace("#productid#", pItem.sku)
						.replace("#name#", pItem.name)
						// .replace("#norm#", norm)
		...
		$(".floor_product_img img")[xigou.events.click](function(){
			var $this = $(this);
			window.location.href = "item.html?tid="+$this.attr("specialid")+"&sku="+$this.attr("productid");
		});
		...
		if(itemType=="2"){//服务类型   手机接收
			...
			$(".ui-border-tel").show();
			$("#telephone").val(data.data.receiveTel);
			$(".total-freight").hide();
			//手机号消除
			if($('#telephone')[0].value != ""){
				$('.div_clear_input1').show();
			}
			$('#telephone').focus(function(e) {
		        var a = $(this).val();
		        if (a && a.length > 0) {
		            $('.div_clear_input1').show();
		        }
...
var checkOrder=function(){
    ...
    if(!(/^1[34578]\d{9}$/.test(phone)) && itemType =="2"){
    	$.tips({
			content:'请输入正确的手机号',
			stayTime:2000,
			type:"warn"
	...
	//地址
	if(!addressnum && itemType!="2"){
		$('.settle-address-info').html('<span>添加收货地址</span>');
		// xigou.alert("请先添加收货地址!");
		$.dialog({
        title:'',
        content:'请先添加收货地址!',
        button:["确认"]
