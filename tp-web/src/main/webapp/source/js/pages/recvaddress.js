...
function setSetRecvaddress(data){
	...
	if (selectType) {
			...
			if (aid != -1) {
				for (var i = 0; i < data.length; i++) {
					if (data[i].aid == aid){
						...
						if(selectType == "1"){
							window.location.href='settle.html?selectType=1';
		...
		if(selectType == "1"){
			$('.goBack').attr('href', 'settle.html?selectType=1');
		}else if(selectType == "2"){
			$('.goBack').attr('href', 'settle.html?selectType=2');
		}
		$(".recvaddress-add")[xigou.events.click](function(event) {
			if(selectType == "1"){
				window.location.href='addaddress.html?selectType=1';
			}else if(selectType == "2"){
				window.location.href='addaddress.html?selectType=2';
			}
		});
	}
}