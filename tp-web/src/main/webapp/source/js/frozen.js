...
function Scroll(el, options) {
	...
	if (this.options.role === 'slider') {
		...
		this.scroller = $('.ui-slider-content')[this.options.defaultIndex];
...
Scroll.prototype = {
	...
	_autoplay: function(flag,num) {
		var self = this,
			curPage = self.currentPage;
		var rag = new RegExp("[0-"+(self.count-1)+"]");//非自动播放时匹配选项卡最大个数
		self.currentPage = rag.test(num)?num:(self.currentPage >= self.count-1 ? 0 : ++self.currentPage);
