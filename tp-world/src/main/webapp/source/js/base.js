...
(function($) {
	...
	$.fn.imglazyload = function(opts) {
		...
		function isInViewport(offset) { //图片出现在可视区的条件
			...
			var isshow= viewTop >= offset[OFFSET.img[0]] - opts.threshold - viewHeight && viewTop <= offset[OFFSET.img[0]] + offset[OFFSET.img[1]];
			return isshow;
		}
		...
		function _hide() { //检测图片是否出现在可视区，并对满足条件的开始加载
			...
			for (i = pedding.length; i--;) {
				...
				var viewOffset = isWindow ? window : $viewPort.offset(),
					viewTop = viewOffset[OFFSET.win[0]],
					viewHeight = viewOffset[OFFSET.win[1]];
				if(offset[OFFSET.img[0]]<(viewTop-opts.threshold) 
				&& offset[OFFSET.img[0]]>(viewTop-opts.threshold-2*viewHeight)){
					$image.attr('src','data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC');
				}
				if(offset[OFFSET.img[0]]>(viewTop+opts.threshold) 
				&& offset[OFFSET.img[0]]<(viewTop+opts.threshold+2*viewHeight)){
					$image.attr('src','data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC');
...
if( typeof tracking_collector == 'undefined' ) tracking_collector = 'http://tracking.51seaco.com';
