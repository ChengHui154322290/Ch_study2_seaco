...
(function ($) {
    ...
    $.fn.mdatetimer = function(opts){
		var defaults = {
			...
			years : [2016,2017,2018,2019,2020,2021], //年份，数组或字符串'2000~2015'
			nowbtn : true,
			onOk : null,
			onCancel : null,
			maxDate:new Date(),
			minDate:new Date(Date.parse('2013/01/01'))
		};
		...
		input.each(function(index, element){
			...
			var picker = {
				init : function(){
					...
					var yearStr = '';
					var maxYear = parseInt(option.maxDate.getFullYear());
					var minYear = parseInt(option.minDate.getFullYear());
					...
					//初始化month
					var monthStr = '';
					var maxMonth = parseInt(option.maxDate.getMonth());
					var minMonth = parseInt(option.minDate.getMonth());
					var minYM = parseInt(minYear+''+((minMonth+1)>9?(minMonth+1):'0'+(minMonth+1)));
					var maxYM = parseInt(maxYear+''+((maxMonth+1)>9?(maxMonth+1):'0'+(maxMonth+1)));
					for(var j=1; j<=12; j++){
						var sel = (j-1)==dMonth ? 'selected' : '';
						var cur_date = parseInt(dYear+''+(j>9?j:'0'+j));
						if(cur_date>=minYM && cur_date<=maxYM){
					...
					var maxDay = parseInt(option.maxDate.getDate());
					var minDay = parseInt(option.minDate.getDate());
					var minYMD = parseInt(minYM+''+(minDay>9?minDay:'0'+minDay));
					var maxYMD = parseInt(maxYM+''+(maxDay>9?maxDay:'0'+maxDay));
					var curYM  = parseInt(dYear+''+(dMonth>8?(dMonth+1):'0'+(dMonth+1)));
					for(var k=1; k<=defaultDays; k++){
						var sel = k==dDate ? 'selected' : '';
						var cur_date = parseInt(curYM+''+(k>9?k:'0'+k));
						if(cur_date>=minYMD && cur_date<=maxYMD){
					...
					yearScroll.on('scrollEnd', function(){
						_this.updateSelected(mpYear, this);
						_this.updateMonth(mpYear,mpMonth, mpDay);
				...
				//滚动的时候动态调节月分
				updateMonth : function(mpYear, mpMonth, mpDay){
					var _this = this;
					...
					var monthStr = '<li class="mt_note">选择月份</li><li></li>';
					var minYM = parseInt(minYear+''+((minMonth+1)>9?(minMonth+1):'0'+(minMonth+1)));
					var maxYM = parseInt(maxYear+''+((maxMonth+1)>9?(maxMonth+1):'0'+(maxMonth+1)));
					for(var j=1; j<=12; j++){
						var sel = (j-1)==checkedMonth ? 'selected' : '';
						var cur_date = parseInt(checkedYear+''+(j>9?j:'0'+j));
						if(cur_date>=minYM && cur_date<=maxYM){
							monthStr += '<li class="'+sel+'" data-month="'+(j-1)+'">'+F.preZero(j)+'月</li>';
						}
					}
					monthStr += '<li></li><li></li>';
					mpMonth.find('ul').html(monthStr);

					//内容改变后，iscroll的滚动会发生错误，所以在此将日期scorll重新初始化一遍
					this.monthScroll.destroy();
					this.monthScroll = new IScroll('.mt_poppanel_'+id+' .mt_month', scrollConf);
					this.monthScroll.on('scroll', function(){
						_this.updateSelected(mpMonth, this);
					});
					...
					//然后给day重新选择
					setTimeout(function(){
						var monthEl = mpMonth.find('li[data-month="'+checkedMonth+'"]');
						if(monthEl.length>0){
							_this.checkMonth(monthEl);
						}
					},10);
				},
				...
				updateDay : function(mpYear, mpMonth, mpDay){
					...
					var minYMD = parseInt(minYM+''+(minDay>9?minDay:'0'+minDay));
					var maxYMD = parseInt(maxYM+''+(maxDay>9?maxDay:'0'+maxDay));
					var curYM  = parseInt(checkedYear+''+(checkedMonth>8?(checkedMonth+1):'0'+(checkedMonth+1)));
