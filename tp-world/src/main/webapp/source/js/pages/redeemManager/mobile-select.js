...
(function(root, factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		root.MobileSelectArea = factory(window.Zepto || window.$ || $);
	}
	...
	MobileSelectArea.prototype = {
		init: function(settings) {
			...
			this.text = this.settings.text || this.trigger.attr('value').split(' ') || ['', '', ''];
		...
		submit: function() {
			...
			if (this.trigger[0].nodeType == 1) {
				//input
				this.trigger.text(this.text.slice(0,this.level).join(this.separator));
				this.trigger.attr('data-value', this.value.slice(0,this.level).join(','));
			}
			this.trigger.next().val(this.value.slice(0,this.level).join(','));
