$(function(){
    var back_url = decodeURIComponent(xigou.getQueryString('back_url')) || 'http://m.51seaco.com/index.html';
    ...
    if (openId != null && !xigou.getQueryString('getNickName')) {
        if(/^.*\.html?$/.test(back_url)){
    ...
    if(openId && xigou.getQueryString('getNickName')){
        var wxImage = xigou.getSessionStorage("wxImage");
        var wxnickname = xigou.getSessionStorage("wxnickname");
        if(!wxImage){
    ...
    xigou.activeUser.getcode({
        ...
        callback:function(response){
            var back_url = response.data.codeurl || 'http://m.51seaco.com/index.html';
            if (back_url){
                window.location.href = back_url;
            }
        }
            ...
            callback: function(response) {
                if (response && response.code == '0') {
                    if (response.data.openid) {
                        xigou.setSessionStorage("openid", response.data.openid);
                    }
                    if (response.data.headimgurl) {
                        xigou.setSessionStorage("wxImage", response.data.headimgurl);
                    }
                    if (response.data.nickname) {
                        xigou.setSessionStorage("wxnickname", response.data.nickname);
                    }
                }
