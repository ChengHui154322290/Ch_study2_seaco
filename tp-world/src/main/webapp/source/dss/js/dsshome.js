...
$(function () {
	...
	$("#myDsshome").on("click",function(){
		xigou.removeSessionStorage('shareShop');
		xigou.removeSessionStorage('dssName');
	})
	/*  弹出提示框    
...
function InitDssAccountPage(json) {
	...
	if(status == 0){
		...
		$("#orders_btn_pay").attr("orderprice", json.data.payamount);
	}
		...
		callback: function(response, status) {
			if (status == xigou.dictionary.success) {
				if(response != null && response != "undefined"){
					...
					if(!isWeiXin()){
						$(".nowx_tip").html("你也可以手动复制链接：<br/>http://w.51seaco.com/dssinvite.html?i=" + shopMobileVal);
					}
				}
				else {
...
$("#orders_btn_pay").on("click",function(){
	...
	xigou.setSessionStorage("payid",payid);
	xigou.setSessionStorage("payway",payway);
	xigou.setSessionStorage("orderprice",orderprice);
	
	console.log(ordercode);
	console.log(payid);
	console.log(payway);
	window.location.href="../dss/openDiscount.html";
})

//PreviewImage  预览
/*function wxPreviewImage(){
	var imgCodeURL = $('.div_dsslogo img').attr('src');
	if (imgCodeURL){
