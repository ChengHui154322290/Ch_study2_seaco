...
import com.tp.world.helper.SwitchHelper;
...
public class ShoppingCartConvert {
	...
	public static ProductVO convertCartLine(CartItemInfo cartItemInfo,boolean isCart){
		...
		if(null != cartItemInfo){
			...
			item.setItemspecs(convertProductSpecs(cartItemInfo.getSalePropertyList())); //规格信息
		}
		return item;
	}
	...
	public static ProductVO convertCartLine(OrderItem orderItem,boolean isCart){
		...
		item.setItemspecs(convertProductSpecs(orderItem.getSalePropertyList())); //商品规格信息
		return item;
	}
	...
	public static List<String> convertProductSpecs(List<SalePropertyDTO> salePropertyList){
		List<String> specList = new ArrayList<>();
		for(SalePropertyDTO dto : salePropertyList){
			specList.add(dto.getSpecGroupName() + ":" + dto.getSpecName());
		}
		return specList;
	}
}
