...
import com.tp.world.convert.AddressConvert;
...
public class AddressAO {
    ...
    public MResultVO<AddressDetailVO> modifyAddress(QueryAddress address) {
        try {
            if (!VersionHelper.before130Version(address)) {
               // AssertUtil.notEmpty(address.getFrontimg(), MResultInfo.USER_AUTH_FRONT_IMG_NULL);
               // AssertUtil.notEmpty(address.getBackimg(), MResultInfo.USER_AUTH_BACK_IMG_NULL);
