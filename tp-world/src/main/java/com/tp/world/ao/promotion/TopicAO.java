...
import com.tp.world.convert.TopicConvert;
...
public class TopicAO {
     ...
     * @return
     */
    public MResultVO<TopicDetailVO> getTopicDetail(Long tid) {
        try {
            String shareUrl = propertiesHelper.shareTopicUrl.trim().replace("TID", tid.toString());
            TopicItemBrandCategoryDTO topic = singleTempleProxy.getISingleBusTemService().loadTopiInHtmlApp(tid);
            if (null != topic) {
                TopicDetailVO t = TopicConvert.convertTopicDetail(topic.getTopic(), shareUrl);
                /**
                 * 供应商店铺改造  start
                 */
                ...
                if (supplierId != null && supplierId != 0 && type.equals(TopicType.SUPPLIER_SHOP.ordinal())) {//类型为店铺时
                    ...
                    if (supplierShop != null) {
                        ...
                        t.setShareurl("https://m.51seaco.com/group_shop.html?id=" + tid);
                        t.setPrestime(supplierShop.getBusinessTime());
                        t.setAddr(supplierShop.getShopAddress());
                        t.setTel(supplierShop.getShopTel());
                        t.setNotice(StringUtils.isBlank(topic.getTopic().getRemark()) ? "欢迎光临本店!" : topic.getTopic().getRemark());
                        t.setPoint(StringUtils.isBlank(topic.getTopic().getTopicPoint()) ? "欢迎光临本店!" : topic.getTopic().getTopicPoint());
                        t.setLatitude(null);
                        t.setLatitude(null);
                    }
                }
