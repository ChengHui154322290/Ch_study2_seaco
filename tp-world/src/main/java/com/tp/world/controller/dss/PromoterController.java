 ...
 * 
 */
package com.tp.world.controller.dss;
...
import com.tp.world.ao.system.FileAO;
...
public class PromoterController {
	...
	public String login(HttpServletRequest request){
		try{
			...
			Long userId = usr.getUid();
			MResultVO<DssLoginVO> result = promoterAO.promoterLogin(userId, promoter.getToken());
			if( result.getCode().equals(MResultInfo.SUCCESS .code)  
					&&  result.getData().getIsshopdss()!=null && result.getData().getIsshopdss().equals(1)){	// 如果已经注册为分销店铺，直接登录
				HttpSession session = request.getSession();
				session.setAttribute("dssUser", usr.getTel());
			}
			if(log.isInfoEnabled()){
				log.info("[API接口 -推广员登录 返回值] = {}",JsonUtil.convertObjToStr(result));
			}
	...
	public String promoterinfo(HttpServletRequest request){
		try{
			...
			MResultVO<PromoterInfoVO> result = promoterAO.getPromoterInfo(promoter);
			...
			if("1".equals( promoter.getType())){//店铺推广
//				if( StringUtils.isBlank(result.getData().getShareImpagePath() )){
//					ResultInfo<PromoterInfo> invitePromoterInfo = null;
//					PromoterInfoVO promoterInfo = result.getData();
//					ResultInfo<PromoterInfo> resultPromoterInfo = promoterInfoProxy.queryById(promoter.getPromoterid());  
//					String mobile=resultPromoterInfo.getData().getMobile();
//					invitePromoterInfo = dSSUserAO.getPromoterInfo(mobile);
//					PromoterInfo	promoterInfoForSave = resultPromoterInfo.getData();
//					String shareImagePath=getShareImageData(propertiesHelper.shareErweimaUrl + mobile );
//					promoterInfoForSave.setShareImagePath(shareImagePath);
//					dSSUserAO.updateAccountInfo(promoterInfoForSave);//更新二维码信息
//					result.getData().setShareImpagePath(shareImagePath);
//					log.info("[API接口 -获取分销人员店铺网址 ] = {}", propertiesHelper.shareErweimaUrl + mobile );
//				}else{
//					result.getData().setShareImpagePath(result.getData().getShareImpagePath());//带背景的二维码图片
//				}
				ResultInfo<PromoterInfo> invitePromoterInfo = null;
				PromoterInfoVO promoterInfo = result.getData();
				ResultInfo<PromoterInfo> resultPromoterInfo = promoterInfoProxy.queryById(promoter.getPromoterid());  
				...
				invitePromoterInfo = dSSUserAO.getPromoterInfo(mobile);
				PromoterInfo	promoterInfoForSave = resultPromoterInfo.getData();
				String shareImagePath=getShareImageData(propertiesHelper.shareErweimaUrl + mobile +"&dssName="+invitePromoterInfo.getData().getNickName());
				promoterInfoForSave.setShareImagePath(shareImagePath);
//				dSSUserAO.updateAccountInfo(promoterInfoForSave);//更新二维码信息
				result.getData().setShareImpagePath(shareImagePath);
				log.info("[API接口 -获取分销人员店铺网址 ] = {}", propertiesHelper.shareErweimaUrl + mobile+"&dssName="+invitePromoterInfo.getData().getNickName() );
			}
//			
...
//			if( StringUtil.isNotBlank(promoter.getEmail())){
//				AssertUtil.notValid(promoter.getEmail(), ValidFieldType.EMAIL);				
//			}
//			if( StringUtil.isNotBlank(promoter.getAlipay())){
//				if( !VerifyUtil.verifyEmail(promoter.getAlipay()) && !VerifyUtil.verifyTelephone(promoter.getAlipay())  ){
//					throw new MobileException(MResultInfo.ALIPAY_NOT_VALID);
//				}
//			}
...
//			MResultVO<MResultInfo> result = promoterAO.withdrawApply(promoter);
	...
	public String setShelves(HttpServletRequest request){
		try{
			...
			Long promoterId = promoterAO.authPromoter(promoterTopic.getToken(), Integer.valueOf(promoterTopic.getType()));			
	...
	public String getTopics(HttpServletRequest request){
		try{
			...
			promoter.setPromoterid(promoterId);
			promoter.setTopictype( promoterTopic.getTopictype());
			MResultVO< List<TopicInfoVO> > result  = promoterAO.getTopics(promoter);
			if(log.isInfoEnabled()){
	...
	public String getTopicItems(HttpServletRequest request){
		try{
			...
			MResultVO< Page<PromoterTopicItemVO> > result  = promoterAO.getTopicItems(pTopic);
			if(log.isInfoEnabled()){
