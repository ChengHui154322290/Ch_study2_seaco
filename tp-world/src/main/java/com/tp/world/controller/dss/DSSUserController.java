			...
			if (StringUtil.isNotBlank(jsonStr)) {
				...
				if (promoter != null && (StringUtil.isNotBlank(promoter.getToken()))) {
					...
					if (invitePromoterInfo != null ) {//全部实时生成分享图片
						...
						String shareImagePath=getShareImageData(propertiesHelper.shareErweimaUrl + mobile +"&dssName="+invitePromoterInfo.getNickName());
...
//						dSSUserAO.updateAccountInfo(promoterInfo);//更新二维码信息
						log.info("[API接口 -获取分销人员店铺网址 ] = {}", propertiesHelper.shareErweimaUrl + mobile +"&dssName="+invitePromoterInfo.getNickName());
					}else{
					...
					if(promoter.getUserMobile() != null){
						...
						if (invitePromoterInfo != null) {
							promoterInfo = invitePromoterInfo;
							result.put("img", getShareImageData(propertiesHelper.shareErweimaUrl + promoter.getUserMobile() +"&dssName="+invitePromoterInfo.getNickName()));
							log.info("[API接口 -获取分销人员店铺网址 ] = {}", propertiesHelper.shareErweimaUrl + promoter.getUserMobile() +"&dssName="+invitePromoterInfo.getNickName());
						}
					}
				}
			}
		...
		try {
			...
			if (StringUtil.isNotBlank(jsonStr)) {
				...
				if (promoter != null && StringUtil.isNotBlank(promoter.getToken()) ) {
					...
					if (invitePromoterInfo != null ) {
						promoterInfo = invitePromoterInfo;
						String img=getImageData(propertiesHelper.shareErweimaUrl + mobile+"&dssName="+invitePromoterInfo.getNickName() );
						result.put("img", img);
						log.info("[API接口 -获取分销人员店铺网址 ] = {}", propertiesHelper.shareErweimaUrl + mobile +"&dssName="+invitePromoterInfo.getNickName());
					}
				}
